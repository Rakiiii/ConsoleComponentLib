...
template <template <class...> class container,class ViewHolder , class type>
class RecyclerView : public View
{
	private:

		//список всех viewholder содержащихся в recyclerview
		container<type> collectionOfElems;
...
{
        if(pos < 0 || pos > collectionOfElems.Size())throw OutOfRange();
        cap = pos;
        return *this;
}
...
{
	if(cap+mv < 0 || cap+mv > collectionOfElems.size())throw OutOfRange();
	cap += mv;
	update();
	return *this;
}
...
void  RecyclerView<container,ViewHolder,type>::update()
{
	...
	if( amountOfShowenElems < ( collectionOfElems.getSize() - cap ) ) outEl = amountOfShowenElems + cap;
	else outEl = collectionOfElems.getSize();
	//позиция вывода элементов из контаинера в recycle view
	int Xpos = 0,Ypos = 0;
	for(int i = 0 ; i < heigh ; i ++)
	{
		for( int j = 0 ; j < width ; j++)elem[i][j];
	}
